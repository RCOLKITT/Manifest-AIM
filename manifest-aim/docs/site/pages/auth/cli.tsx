...
function CLIAuthContent() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event: string, authSession: any) => {
        console.log("Auth event:", event, authSession);
        if (event === "SIGNED_IN" && authSession) {
          await completeAuth(session, authSession.access_token);
        }
      }
    );

    // Check if already logged in
    supabase.auth.getSession().then(({ data: { session: authSession } }: any) => {
      if (authSession) {
  ...
  const completeAuth = async (session: string, accessToken: string) => {
    setState("completing");
    console.log("Completing auth with session:", session);
    ...
    try {
      ...
      console.log("Auth-complete response data:", data);
    ...
    return (
      <div style={styles.container}>
        <div style={styles.card}>
          ...
          <p style={styles.subtitle}>You can close this window and return to your terminal.</p>
          <p style={styles.hint}>The CLI should automatically detect your login.</p>
        </div>
      </div>
    );
  }
...
const styles: Record<string, React.CSSProperties> = {
  ...
  title: { margin: "0 0 8px", fontSize: 24, fontWeight: 600, color: "#1a1a2e" },
  subtitle: { margin: "0 0 24px", color: "#666", fontSize: 14 },
  oauthButtons: { display: "flex", flexDirection: "column" as const, gap: 12, marginBottom: 24 },
  oauthButton: {
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
  ...
  divider: { display: "flex", alignItems: "center", gap: 16, marginBottom: 24, color: "#999", fontSize: 12 },
  ...
  errorBox: { padding: 12, marginBottom: 16, background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 8, color: "#dc2626", fontSize: 14 },
  successIcon: { width: 64, height: 64, margin: "0 auto 16px", borderRadius: "50%", background: "#10b981", color: "#fff", fontSize: 32, lineHeight: "64px" },
  errorIcon: { width: 64, height: 64, margin: "0 auto 16px", borderRadius: "50%", background: "#ef4444", color: "#fff", fontSize: 32, lineHeight: "64px" },
  hint: { color: "#666", fontSize: 14 },
  error: { color: "#dc2626", fontSize: 14, marginBottom: 16 },
  spinner: { width: 32, height: 32, margin: "0 auto 16px", border: "3px solid #e5e7eb", borderTop: "3px solid #6366f1", borderRadius: "50%", animation: "spin 1s linear infinite" },
};

// Export with no SSR to avoid hydration issues
const CLIAuth = dynamic(() => Promise.resolve(CLIAuthContent), { ssr: false });

export default function Page() {
  return <CLIAuth />;
}

// Tell Nextra not to use its theme for this page
